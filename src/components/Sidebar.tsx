...
const allNavItems = [
  ...
  { href: "/dashboard/roles",        label: "Roles & Permissions", icon: Shield,        roles: ["ADMIN"] },
  { href: "/dashboard/workflows",    label: "Workflows",         icon: GitBranch,       roles: ["ADMIN"] },
...
export function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`
          w-64 bg-gray-900 text-white flex flex-col h-screen fixed left-0 top-0 z-40 transition-transform duration-300
          md:translate-x-0
        ...
        <div className="p-6 border-b border-gray-700 shrink-0">
          <div className="flex items-center gap-3">
            ...
            {onClose && (
              <button
                onClick={onClose}
                className="md:hidden p-1 rounded-lg hover:bg-gray-700 text-gray-400 hover:text-white transition-colors"
